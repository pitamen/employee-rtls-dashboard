...
import Sidedetails from './Sidedetails.js';
...
const CentralDash = (props) => {
  ...
  const getEmployeeCount = async () => {
    ...
    try {
      ...
      const response = await fetch(BASE_URL_V2 + "employees/get-employee-count-central", {
        method: 'GET',
        headers: {
  ...
  const fetchUserData = async () => {
    try {
      props.setProgress(10);
      const response = await fetch(BASE_URL_V2 + "locations/live-location-traces-central", {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          // 'Authorization': `Bearer ${sessionStorage.getItem('accessToken')}`
  ...
  return (
    <>
      ...
      <Sidedetails orgResponse={orgResponse} users={users} logData={logDataFromSidedetails} employeeCount={totalEmployeeCount} isFetchingEmployeeCount={isFetchingEmployeeCount} filterData={filterUserAccordingToCategory} isCentral={true} />
      {
