...
import "../my-sass.scss";
import {
  unixTimeStampToISOStringConverter,
  toggleFullScreen,
  ...
  const fetchData = async () => {
    try {
      ...
      if (updatedUserData.length > 0) {
        setNewCenter({
          latitude: updatedUserData[0].lat,
  ...
  return (
    <div className="App">
      <Namebar
        ...
        dashboardName={"DH Field View Dashboard (v0.6.1)"}
      />
      <UserSidedetails
        userDetail={userDetail}
        isFetchingUserDetail={isFetchingUserDetail}
        fetch_enabling={enableDisableLiveTracking}
        isFetchEnabled={fetchEnabled}
        trackedAt={trackedAt}
